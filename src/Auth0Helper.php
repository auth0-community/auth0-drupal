...
use Drupal\auth0\Controller\AuthController;
...
class Auth0Helper {
  ...
  const DEFAULT_USERNAME_CLAIM = 'nickname';
   ...
   */
  public function getNonce($returnTo = null) {
    ...
    $states = $this->tempStore->get(self::STATE);
    if ( ! is_array( $states ) ) {
      $states = array();
    }
    $nonce = $sessionStateHandler->issue();
    $states[$nonce] = $returnTo === null ? '' : $returnTo;
    $this->tempStore->set(self::STATE, $states);
